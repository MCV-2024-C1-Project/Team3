...
def computeVariance(patchCollection, patchSize):
    ...
    Sigma=np.around(covOfImage, decimals=4)
    eigenValue, vect = np.linalg.eig(Sigma)   
    varValue = np.min(eigenValue)
    varValue=np.around(varValue, decimals=4)
...
def noiseLevelEstimation(noisyImage, patchSize=7, confidenceLevel=1-1e-6, numIteration=10):
    ...
    for i in range(numIteration):
        ...
        thresold[i, 0]=np.around(thresold[i, 0], decimals=4) 
        _, numP = np.shape(patchCollection)
...
if __name__=="__main__":
    ...
    for image_path in os.listdir('./data/qsd1_w4'):
        if image_path.endswith('.jpg'):
            ...
            noiseLevel = noiseLevelEstimation(grayImage)
            ...
            if noiseLevel == -1:
                ...
                print("Noise Level of ",image_path," : ",noiseLevel)
