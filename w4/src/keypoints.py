...
def orb_detector(image):
    ...
    orb = cv2.ORB_create(
        ...
        edgeThreshold=31,  # Edge threshold to avoid keypoints near image edges:
        # Default: 31. Typical range: 10 - 50. Lower values capture more points near edges,
        # but could include low-interest keypoints.

        patchSize=31,  # Patch size for computing descriptors:
        ...
        WTA_K=2,  # Number of points to compare in each BRIEF bin:
...
def sift_detector(image):
    ...
    sift = cv2.SIFT_create(
        ...
        sigma=1.6  # Gaussian width for scale-space pyramid:
